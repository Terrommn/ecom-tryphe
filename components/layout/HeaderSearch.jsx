...
export function MobileSearchForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="px-3 py-2">
      <div className="flex items-center gap-2 rounded-full border border-[color:var(--oob-border)] bg-[var(--oob-surface)] px-3 py-1.5">
        <svg
          className="h-4 w-4 shrink-0 text-[var(--oob-muted)]"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          aria-hidden
        ...
        <input
          ...
          type="search"
          placeholder="Buscar..."
          className="flex-1 bg-transparent border-0 p-0 text-sm text-[var(--oob-cream)] placeholder:text-[var(--oob-muted)] focus:ring-0"
        />
      </div>
    </form>
  );

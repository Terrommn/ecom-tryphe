...
export async function addToCart(cartId, lines) {
  const data = await storefrontRequest(CART_LINES_ADD_MUTATION, {
    cartId,
    lines: lines.map((l) => {
      const item = { merchandiseId: l.merchandiseId, quantity: l.quantity };
      if (l.attributes?.length) item.attributes = l.attributes;
      return item;
    }),
  });
  return {
...
/**
 * First N products of a collection by handle. Returns [] if missing or error.
 */
export async function getCollectionProductsFirst(handle, first = 12) {
  const data = await getCollectionPage({ handle, first });
  if (!data?.products?.edges?.length) return [];
  return data.products.edges.map((e) => e.node);
}
